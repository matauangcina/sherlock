...
public class MonotonicCurveFit extends CurveFit {
    ...
    public void getPos(double t, double[] v) {
        ...
        int n = dArr.length;
        int dim = this.f31mY[0].length;
        if (this.mExtrapolate) {
            double d = dArr[0];
            if (t <= d) {
                getSlope(d, this.mSlopeTemp);
                for (int j = 0; j < dim; j++) {
                    v[j] = this.f31mY[0][j] + ((t - this.f30mT[0]) * this.mSlopeTemp[j]);
                }
                return;
            } else if (t >= dArr[n - 1]) {
                getSlope(dArr[n - 1], this.mSlopeTemp);
                for (int j2 = 0; j2 < dim; j2++) {
                    v[j2] = this.f31mY[n - 1][j2] + ((t - this.f30mT[n - 1]) * this.mSlopeTemp[j2]);
                }
                return;
            }
        } else if (t <= dArr[0]) {
            for (int j3 = 0; j3 < dim; j3++) {
                v[j3] = this.f31mY[0][j3];
            }
            return;
        } else if (t >= dArr[n - 1]) {
            for (int j4 = 0; j4 < dim; j4++) {
                v[j4] = this.f31mY[n - 1][j4];
            }
            return;
        }
        for (int i = 0; i < n - 1; i++) {
    ...
    public void getPos(double t, float[] v) {
        ...
        if (this.mExtrapolate) {
            ...
            if (t <= d) {
                getSlope(d, this.mSlopeTemp);
                for (int j = 0; j < dim; j++) {
                    v[j] = (float) (this.f31mY[0][j] + ((t - this.f30mT[0]) * this.mSlopeTemp[j]));
                }
                return;
            } else if (t >= dArr[n - 1]) {
                getSlope(dArr[n - 1], this.mSlopeTemp);
                for (int j2 = 0; j2 < dim; j2++) {
                    v[j2] = (float) (this.f31mY[n - 1][j2] + ((t - this.f30mT[n - 1]) * this.mSlopeTemp[j2]));
                }
                return;
            }
        } else if (t <= dArr[0]) {
            for (int j3 = 0; j3 < dim; j3++) {
                v[j3] = (float) this.f31mY[0][j3];
            }
            return;
        } else if (t >= dArr[n - 1]) {
            for (int j4 = 0; j4 < dim; j4++) {
                v[j4] = (float) this.f31mY[n - 1][j4];
            }
            return;
        }
        for (int i = 0; i < n - 1; i++) {
    ...
    public double getPos(double t, int j) {
        ...
        if (this.mExtrapolate) {
            double d = dArr[0];
            if (t <= d) {
                return this.f31mY[0][j] + ((t - d) * getSlope(d, j));
            }
            if (t >= dArr[n - 1]) {
                return this.f31mY[n - 1][j] + ((t - dArr[n - 1]) * getSlope(dArr[n - 1], j));
            }
        } else if (t <= dArr[0]) {
            return this.f31mY[0][j];
        } else {
            if (t >= dArr[n - 1]) {
                return this.f31mY[n - 1][j];
            }
        }
        for (int i = 0; i < n - 1; i++) {
            double[] dArr2 = this.f30mT;
