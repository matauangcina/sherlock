...
public class ConstraintAttribute {
    ...
    public boolean isContinuous() {
        switch (C01751.f57x66adad53[this.mType.ordinal()]) {
            case 1:
            case 2:
            case 3:
                return false;
    ...
    public static void setAttributes(View view, HashMap<String, ConstraintAttribute> map) {
        ...
        for (String name : map.keySet()) {
            ...
            try {
                switch (C01751.f57x66adad53[constraintAttribute.mType.ordinal()]) {
                    case 1:
    ...
    public static void parse(Context context, XmlPullParser parser, HashMap<String, ConstraintAttribute> custom) {
        ...
        TypedArray a = context.obtainStyledAttributes(attributeSet, C0177R.styleable.CustomAttribute);
        String name = null;
        ...
        for (int i = 0; i < N; i++) {
            ...
            if (attr == C0177R.styleable.CustomAttribute_attributeName) {
                name = a.getString(attr);
                if (name != null && name.length() > 0) {
                    name = Character.toUpperCase(name.charAt(0)) + name.substring(1);
                }
            } else if (attr == C0177R.styleable.CustomAttribute_methodName) {
                method = true;
                name = a.getString(attr);
            } else if (attr == C0177R.styleable.CustomAttribute_customBoolean) {
                value = Boolean.valueOf(a.getBoolean(attr, false));
                type = AttributeType.BOOLEAN_TYPE;
            } else if (attr == C0177R.styleable.CustomAttribute_customColorValue) {
                type = AttributeType.COLOR_TYPE;
                value = Integer.valueOf(a.getColor(attr, 0));
            } else if (attr == C0177R.styleable.CustomAttribute_customColorDrawableValue) {
                type = AttributeType.COLOR_DRAWABLE_TYPE;
                value = Integer.valueOf(a.getColor(attr, 0));
            } else if (attr == C0177R.styleable.CustomAttribute_customPixelDimension) {
            ...
            } else if (attr == C0177R.styleable.CustomAttribute_customDimension) {
                type = AttributeType.DIMENSION_TYPE;
                value = Float.valueOf(a.getDimension(attr, 0.0f));
            } else if (attr == C0177R.styleable.CustomAttribute_customFloatValue) {
                type = AttributeType.FLOAT_TYPE;
                value = Float.valueOf(a.getFloat(attr, Float.NaN));
            } else if (attr == C0177R.styleable.CustomAttribute_customIntegerValue) {
                type = AttributeType.INT_TYPE;
                value = Integer.valueOf(a.getInteger(attr, -1));
            } else if (attr == C0177R.styleable.CustomAttribute_customStringValue) {
                type = AttributeType.STRING_TYPE;
                value = a.getString(attr);
            } else if (attr == C0177R.styleable.CustomAttribute_customReference) {
