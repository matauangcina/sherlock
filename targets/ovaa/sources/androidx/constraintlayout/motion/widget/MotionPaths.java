...
public class MotionPaths implements Comparable<MotionPaths> {
    ...
    void initCartesian(KeyPosition c, MotionPaths startTimePoint, MotionPaths endTimePoint) {
        ...
        float f5 = startTimePoint.f54x;
        float startCenterX = f5 + (f2 / 2.0f);
        float position2 = startTimePoint.f55y;
        float startCenterY = position2 + (f4 / 2.0f);
        float endCenterX = endTimePoint.f54x + (f / 2.0f);
        float endCenterY = endTimePoint.f55y + (f3 / 2.0f);
        float pathVectorX = endCenterX - startCenterX;
        float pathVectorY = endCenterY - startCenterY;
        this.f54x = (int) ((f5 + (pathVectorX * position)) - ((scaleX * scaleWidth) / 2.0f));
        this.f55y = (int) ((position2 + (pathVectorY * position)) - ((scaleY * scaleHeight) / 2.0f));
        ...
        this.mMode = 0;
        this.f54x = (int) (((startTimePoint.f54x + (pathVectorX * dxdx)) + (pathVectorY * dxdy)) - ((scaleX * scaleWidth) / 2.0f));
        this.f55y = (int) (((startTimePoint.f55y + (pathVectorX * dydx)) + (pathVectorY * dydy)) - ((scaleY * scaleHeight) / 2.0f));
    ...
    public void setupRelative(MotionController mc, MotionPaths relative) {
        double dx = ((this.f54x + (this.width / 2.0f)) - relative.f54x) - (relative.width / 2.0f);
        double dy = ((this.f55y + (this.height / 2.0f)) - relative.f55y) - (relative.height / 2.0f);
        this.mRelativeToController = mc;
        this.f54x = (float) Math.hypot(dy, dx);
        if (Float.isNaN(this.mRelativeAngle)) {
            this.f55y = (float) (Math.atan2(dy, dx) + 1.5707963267948966d);
        } else {
    ...
    void initPath(KeyPosition c, MotionPaths startTimePoint, MotionPaths endTimePoint) {
        ...
        float startCenterX = (startTimePoint.width / 2.0f) + startTimePoint.f54x;
        float startCenterY = startTimePoint.f55y + (startTimePoint.height / 2.0f);
        float endCenterX = endTimePoint.f54x + (endTimePoint.width / 2.0f);
        float endCenterY = endTimePoint.f55y + (endTimePoint.height / 2.0f);
        float pathVectorX = endCenterX - startCenterX;
        float pathVectorY = endCenterY - startCenterY;
        this.f54x = (int) ((f + (pathVectorX * path)) - ((scaleX * scaleWidth) / 2.0f));
        this.f55y = (int) ((f3 + (pathVectorY * path)) - ((scaleY * scaleHeight) / 2.0f));
        this.width = (int) (f2 + (scaleX * scaleWidth));
        ...
        this.mMode = 1;
        float endCenterY2 = startTimePoint.f54x;
        float f4 = (int) ((endCenterY2 + (pathVectorX * path)) - ((scaleX * scaleWidth) / 2.0f));
        this.f54x = f4;
        float f5 = (int) ((startTimePoint.f55y + (pathVectorY * path)) - ((scaleY * scaleHeight) / 2.0f));
        this.f55y = f5;
        this.f54x = f4 + normalX;
        this.f55y = f5 + normalY;
        this.mAnimateRelativeTo = this.mAnimateRelativeTo;
        this.mKeyFrameEasing = Easing.getInterpolator(c.mTransitionEasing);
        this.mPathMotionArc = c.mPathMotionArc;
    }
    ...
    public void different(MotionPaths points, boolean[] mask, String[] custom, boolean arcMode) {
        boolean diffx = diff(this.f54x, points.f54x);
        boolean diffy = diff(this.f55y, points.f55y);
        int c = 0 + 1;
    ...
    public void setView(float position, View view, int[] toUse, double[] data, double[] slope, double[] cycle, boolean mForceMeasure) {
        ...
        float v_y;
        View view2 = view;
        float v_x2 = this.f54x;
        float v_y2 = this.f55y;
    ...
    public void fillStandard(double[] data, int[] toUse) {
        float[] set = {this.position, this.f54x, this.f55y, this.width, this.height, this.mPathRotate};
        int c = 0;
        for (int i = 0; i < toUse.length; i++) {
