...
public class MotionLayout extends ConstraintLayout implements NestedScrollingParent3 {
    ...
    public void setState(TransitionState newState) {
        ...
        switch (C01615.f52xabc7e4ac[oldState.ordinal()]) {
            case 1:
            case 2:
    ...
    public static /* synthetic */ class C01615 {
        ...
        static {
            ...
            try {
                f52xabc7e4ac[TransitionState.SETUP.ordinal()] = 2;
            } catch (NoSuchFieldError e2) {
            }
            try {
                f52xabc7e4ac[TransitionState.MOVING.ordinal()] = 3;
            } catch (NoSuchFieldError e3) {
            }
            try {
                f52xabc7e4ac[TransitionState.FINISHED.ordinal()] = 4;
    ...
    private static class MyTracker implements MotionTracker {
        ...
        /* renamed from: me */
        private static MyTracker f53me = new MyTracker();
        ...
        public static MyTracker obtain() {
            f53me.tracker = VelocityTracker.obtain();
            return f53me;
        }
    ...
    */
    ...
    private void init(AttributeSet attrs) {
        ...
        if (attrs != null) {
            TypedArray a = getContext().obtainStyledAttributes(attrs, C0177R.styleable.MotionLayout);
            int N = a.getIndexCount();
            boolean apply = true;
            for (int i = 0; i < N; i++) {
                int attr = a.getIndex(i);
                if (attr == C0177R.styleable.MotionLayout_layoutDescription) {
                    int n = a.getResourceId(attr, -1);
                    this.mScene = new MotionScene(getContext(), this, n);
                } else if (attr == C0177R.styleable.MotionLayout_currentState) {
                    this.mCurrentState = a.getResourceId(attr, -1);
                } else if (attr == C0177R.styleable.MotionLayout_motionProgress) {
                    this.mTransitionGoalPosition = a.getFloat(attr, 0.0f);
                    this.mInTransition = true;
                } else if (attr == C0177R.styleable.MotionLayout_applyMotionScene) {
                    apply = a.getBoolean(attr, apply);
                } else if (attr == C0177R.styleable.MotionLayout_showPaths) {
                    if (this.mDebugPath == 0) {
                        this.mDebugPath = a.getBoolean(attr, false) ? 2 : 0;
                    }
                } else if (attr == C0177R.styleable.MotionLayout_motionDebug) {
    ...
    public void updateStateAnimate(int stateId, ConstraintSet set, int duration) {
        if (this.mScene != null && this.mCurrentState == stateId) {
            updateState(C0177R.C0180id.view_transition, getConstraintSet(stateId));
            setState(C0177R.C0180id.view_transition, -1, -1);
            updateState(stateId, set);
            MotionScene.Transition tmpTransition = new MotionScene.Transition(-1, this.mScene, C0177R.C0180id.view_transition, stateId);
