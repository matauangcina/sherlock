...
public class FlatBufferBuilder {
    ...
    public FlatBufferBuilder(int initial_size, ByteBufferFactory bb_factory, ByteBuffer existing_bb, Utf8 utf8) {
        ...
        if (existing_bb != null) {
            this.f83bb = existing_bb;
            existing_bb.clear();
            this.f83bb.order(ByteOrder.LITTLE_ENDIAN);
        } else {
            this.f83bb = bb_factory.newByteBuffer(initial_size);
        }
        this.utf8 = utf8;
        this.space = this.f83bb.capacity();
    }
    ...
    public int offset() {
        return this.f83bb.capacity() - this.space;
    }

    public void pad(int byte_size) {
        for (int i = 0; i < byte_size; i++) {
            ByteBuffer byteBuffer = this.f83bb;
            int i2 = this.space - 1;
            this.space = i2;
    ...
    public void prep(int size, int additional_bytes) {
        ...
        int align_size = ((~((this.f83bb.capacity() - this.space) + additional_bytes)) + 1) & (size - 1);
        while (this.space < align_size + size + additional_bytes) {
            int old_buf_size = this.f83bb.capacity();
            ByteBuffer old = this.f83bb;
            ByteBuffer growByteBuffer = growByteBuffer(old, this.bb_factory);
            this.f83bb = growByteBuffer;
            if (old != growByteBuffer) {
                this.bb_factory.releaseByteBuffer(old);
            }
            this.space += this.f83bb.capacity() - old_buf_size;
        }
        pad(align_size);
    }
    ...
    public ByteBuffer createUnintializedVector(int elem_size, int num_elems, int alignment) {
        ...
        ByteBuffer copy = this.f83bb.slice().order(ByteOrder.LITTLE_ENDIAN);
        copy.limit(length);
        return copy;
    }
    ...
    public <T extends Table> int createSortedVectorOfTables(T obj, int[] offsets) {
        obj.sortTables(offsets, this.f83bb);
    ...
    public int createString(CharSequence s) {
        ...
        this.space = i;
        byteBuffer.position(i);
        this.utf8.encodeUtf8(s, this.f83bb);
        return endVector();
    }
    ...
    public int createByteVector(byte[] arr, int offset, int length) {
        ...
        this.space = i;
        byteBuffer.position(i);
        this.f83bb.put(arr, offset, length);
        return endVector();
    }
    ...
    public int createByteVector(ByteBuffer byteBuffer) {
        ...
        startVector(1, length, 1);
        ByteBuffer byteBuffer2 = this.f83bb;
        int i = this.space - length;
        this.space = i;
        byteBuffer2.position(i);
        this.f83bb.put(byteBuffer);
        return endVector();
    }
    ...
    public int endTable() {
        ...
        loop2: while (true) {
            ...
            int vt1 = this.f83bb.capacity() - this.vtables[i3];
            int vt2 = this.space;
            short len = this.f83bb.getShort(vt1);
            if (len == this.f83bb.getShort(vt2)) {
                for (int j = 2; j < len; j += 2) {
                    if (this.f83bb.getShort(vt1 + j) != this.f83bb.getShort(vt2 + j)) {
                        break;
                    }
                }
        ...
        if (existing_vtable != 0) {
            int capacity = this.f83bb.capacity() - vtableloc;
            this.space = capacity;
            this.f83bb.putInt(capacity, existing_vtable - vtableloc);
        } else {
    ...
    public void required(int table, int field) {
        int table_start = this.f83bb.capacity() - table;
        int vtable_start = table_start - this.f83bb.getInt(table_start);
        boolean ok = this.f83bb.getShort(vtable_start + field) != 0;
        if (!ok) {
    ...
    protected void finish(int root_table, boolean size_prefix) {
        ...
        if (size_prefix) {
            addInt(this.f83bb.capacity() - this.space);
        }
        this.f83bb.position(this.space);
        this.finished = true;
    }
    ...
    public byte[] sizedByteArray(int start, int length) {
        ...
        this.f83bb.position(start);
        this.f83bb.get(array);
        return array;
    }

    public byte[] sizedByteArray() {
        return sizedByteArray(this.space, this.f83bb.capacity() - this.space);
    }

    public InputStream sizedInputStream() {
        finished();
        ByteBuffer duplicate = this.f83bb.duplicate();
        duplicate.position(this.space);
        duplicate.limit(this.f83bb.capacity());
