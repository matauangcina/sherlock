...
public class FlexBuffersBuilder {
    ...
    /* renamed from: bb */
    private final ReadWriteBuf f86bb;
    ...
    public FlexBuffersBuilder(ReadWriteBuf bb, int flags) {
        ...
        this.keyComparator = new Comparator<Value>() { // from class: androidx.emoji2.text.flatbuffer.FlexBuffersBuilder.1
            ...
            public int compare(Value o1, Value o2) {
                ...
                int io = o2.key;
                do {
                    c1 = FlexBuffersBuilder.this.f86bb.get(ia);
                    c2 = FlexBuffersBuilder.this.f86bb.get(io);
                    if (c1 == 0) {
                        return c1 - c2;
                    }
                    ia++;
    ...
    private int putKey(String key) {
        ...
        int pos = this.f86bb.writePosition();
        if ((this.flags & 1) != 0) {
            Integer keyFromPool = this.keyPool.get(key);
            if (keyFromPool == null) {
                byte[] keyBytes = key.getBytes(StandardCharsets.UTF_8);
                this.f86bb.put(keyBytes, 0, keyBytes.length);
                this.f86bb.put((byte) 0);
        ...
        this.f86bb.put(keyBytes2, 0, keyBytes2.length);
        this.f86bb.put((byte) 0);
        this.keyPool.put(key, Integer.valueOf(pos));
        return pos;
    }
    ...
    private Value writeBlob(int key, byte[] blob, int type, boolean trailing) {
        ...
        int sloc = this.f86bb.writePosition();
        this.f86bb.put(blob, 0, blob.length);
        if (trailing) {
            this.f86bb.put((byte) 0);
        }
    ...
    private int align(int alignment) {
        ...
        int padBytes = Value.paddingBytes(this.f86bb.writePosition(), byteWidth);
        while (true) {
    ...
    private void writeInt(long value, int byteWidth) {
        switch (byteWidth) {
            case 1:
                this.f86bb.put((byte) value);
                return;
            case 2:
                this.f86bb.putShort((short) value);
                return;
            case 4:
                this.f86bb.putInt((int) value);
                return;
            case 8:
                this.f86bb.putLong(value);
                return;
            default:
                return;
        }
    ...
    public ByteBuffer finish() {
        ...
        int byteWidth = align(this.stack.get(0).elemWidth(this.f86bb.writePosition(), 0));
        writeAny(this.stack.get(0), byteWidth);
        this.f86bb.put(this.stack.get(0).storedPackedType());
        this.f86bb.put((byte) byteWidth);
        this.finished = true;
        return ByteBuffer.wrap(this.f86bb.data(), 0, this.f86bb.writePosition());
    }
    ...
    private Value createVector(int key, int start, int length, boolean typed, boolean fixed, Value keys) {
        ...
        if (!fixed || typed) {
            ...
            int prefixElems = 1;
            if (keys != null) {
                bitWidth = Math.max(bitWidth, keys.elemWidth(this.f86bb.writePosition(), 0));
                prefixElems = 1 + 2;
            }
            int vectorType = 4;
            for (int i3 = start; i3 < this.stack.size(); i3++) {
                int elemWidth = this.stack.get(i3).elemWidth(this.f86bb.writePosition(), i3 + prefixElems);
    ...
    private void writeOffset(long val, int byteWidth) {
        int reloff = (int) (this.f86bb.writePosition() - val);
    ...
    private void writeDouble(double val, int byteWidth) {
        if (byteWidth == 4) {
            this.f86bb.putFloat((float) val);
        } else if (byteWidth == 8) {
            this.f86bb.putDouble(val);
        }
    }
    ...
    private Value createKeyVector(int start, int length) {
        ...
        for (int i = start; i < this.stack.size(); i++) {
            int elemWidth = Value.elemWidth(4, 0, this.stack.get(i).key, this.f86bb.writePosition(), i + 1);
            bitWidth = Math.max(bitWidth, elemWidth);
        }
        int byteWidth = align(bitWidth);
        writeInt(length, byteWidth);
