...
public class PathInterpolatorCompat implements Interpolator {
    ...
    /* renamed from: mX */
    private float[] f102mX;

    /* renamed from: mY */
    private float[] f103mY;
    ...
    private void initPath(Path path) {
        ...
        this.f102mX = new float[numPoints];
        this.f103mY = new float[numPoints];
        float[] position = new float[2];
        for (int i = 0; i < numPoints; i++) {
            float distance = (i * pathLength) / (numPoints - 1);
            pathMeasure.getPosTan(distance, position, null);
            this.f102mX[i] = position[0];
            this.f103mY[i] = position[1];
        }
        if (Math.abs(this.f102mX[0]) > 1.0E-5d || Math.abs(this.f103mY[0]) > 1.0E-5d || Math.abs(this.f102mX[numPoints - 1] - 1.0f) > 1.0E-5d || Math.abs(this.f103mY[numPoints - 1] - 1.0f) > 1.0E-5d) {
            throw new IllegalArgumentException("The Path must start at (0,0) and end at (1,1) start: " + this.f102mX[0] + "," + this.f103mY[0] + " end:" + this.f102mX[numPoints - 1] + "," + this.f103mY[numPoints - 1]);
        }
        float prevX = 0.0f;
        int componentIndex = 0;
        int i2 = 0;
        while (i2 < numPoints) {
            float[] fArr = this.f102mX;
    ...
    public float getInterpolation(float t) {
        ...
        int startIndex = 0;
        int endIndex = this.f102mX.length - 1;
        while (endIndex - startIndex > 1) {
            int midIndex = (startIndex + endIndex) / 2;
            if (t < this.f102mX[midIndex]) {
                endIndex = midIndex;
            } else {
                startIndex = midIndex;
            }
        ...
        float xRange = f - f2;
        if (xRange == 0.0f) {
            return this.f103mY[startIndex];
        }
        float tInRange = t - f2;
        float fraction = tInRange / xRange;
        float[] fArr2 = this.f103mY;
