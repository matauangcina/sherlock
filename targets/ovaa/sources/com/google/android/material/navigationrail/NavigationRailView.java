...
public class NavigationRailView extends NavigationBarView {
    ...
    public NavigationRailView(Context context, AttributeSet attrs) {
        this(context, attrs, C0566R.attr.navigationRailStyle);
    }

    public NavigationRailView(Context context, AttributeSet attrs, int defStyleAttr) {
        this(context, attrs, defStyleAttr, C0566R.style.Widget_MaterialComponents_NavigationRailView);
    }
    ...
    public NavigationRailView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        this.topMargin = res.getDimensionPixelSize(C0566R.dimen.mtrl_navigation_rail_margin);
        Context context2 = getContext();
        TintTypedArray attributes = ThemeEnforcement.obtainTintedStyledAttributes(context2, attrs, C0566R.styleable.NavigationRailView, defStyleAttr, defStyleRes, new int[0]);
        int headerLayoutRes = attributes.getResourceId(C0566R.styleable.NavigationRailView_headerLayout, 0);
        if (headerLayoutRes != 0) {
            addHeaderView(headerLayoutRes);
        }
        setMenuGravity(attributes.getInt(C0566R.styleable.NavigationRailView_menuGravity, 49));
        if (attributes.hasValue(C0566R.styleable.NavigationRailView_itemMinHeight)) {
            setItemMinimumHeight(attributes.getDimensionPixelSize(C0566R.styleable.NavigationRailView_itemMinHeight, -1));
        }
        if (attributes.hasValue(C0566R.styleable.NavigationRailView_paddingTopSystemWindowInsets)) {
            this.paddingTopSystemWindowInsets = Boolean.valueOf(attributes.getBoolean(C0566R.styleable.NavigationRailView_paddingTopSystemWindowInsets, false));
        }
        if (attributes.hasValue(C0566R.styleable.NavigationRailView_paddingBottomSystemWindowInsets)) {
            this.paddingBottomSystemWindowInsets = Boolean.valueOf(attributes.getBoolean(C0566R.styleable.NavigationRailView_paddingBottomSystemWindowInsets, false));
        }
        if (attributes.hasValue(C0566R.styleable.NavigationRailView_paddingStartSystemWindowInsets)) {
            this.paddingStartSystemWindowInsets = Boolean.valueOf(attributes.getBoolean(C0566R.styleable.NavigationRailView_paddingStartSystemWindowInsets, false));
        }
        int largeFontTopPadding = getResources().getDimensionPixelOffset(C0566R.dimen.m3_navigation_rail_item_padding_top_with_large_font);
        int largeFontBottomPadding = getResources().getDimensionPixelOffset(C0566R.dimen.m3_navigation_rail_item_padding_bottom_with_large_font);
