...
public final class Http2Reader implements Closeable {
    ...
    public void readConnectionPreface(Handler handler) throws IOException {
        ...
        if (logger2.isLoggable(Level.FINE)) {
            logger2.fine(Util.format("<< CONNECTION %s", connectionPreface.hex()));
        }
    ...
    public boolean nextFrame(boolean requireSettings, Handler handler) throws IOException {
        try {
            ...
            byte type = (byte) (this.source.readByte() & UByte.MAX_VALUE);
            if (requireSettings && type != 4) {
                throw Http2.ioException("Expected a SETTINGS frame but was %s", Byte.valueOf(type));
            }
            byte flags = (byte) (this.source.readByte() & UByte.MAX_VALUE);
            int streamId = this.source.readInt() & Integer.MAX_VALUE;
            Logger logger2 = logger;
            if (logger2.isLoggable(Level.FINE)) {
                logger2.fine(Http2.frameLog(true, streamId, length, type, flags));
            }
            switch (type) {
                case 0:
    ...
    private void readHeaders(Handler handler, int length, byte flags, int streamId) throws IOException {
        ...
        short padding = (flags & 8) != 0 ? (short) (this.source.readByte() & UByte.MAX_VALUE) : (short) 0;
        if ((flags & 32) != 0) {
    ...
    private void readPriority(Handler handler, int streamId) throws IOException {
        ...
        int weight = (this.source.readByte() & UByte.MAX_VALUE) + 1;
    ...
    static int readMedium(BufferedSource source) throws IOException {
        return ((source.readByte() & UByte.MAX_VALUE) << 16) | ((source.readByte() & UByte.MAX_VALUE) << 8) | (source.readByte() & UByte.MAX_VALUE);
    }
