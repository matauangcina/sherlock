...
public class RadialViewGroup extends ConstraintLayout {
    ...
    public RadialViewGroup(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        LayoutInflater.from(context).inflate(C0616R.layout.material_radial_view_group, this);
        ViewCompat.setBackground(this, createBackground());
        TypedArray a = context.obtainStyledAttributes(attrs, C0616R.styleable.RadialViewGroup, defStyleAttr, 0);
        this.radius = a.getDimensionPixelSize(C0616R.styleable.RadialViewGroup_materialCircleRadius, 0);
    ...
    public void updateLayoutParams() {
        ...
        for (int i = 0; i < getChildCount(); i++) {
            ...
            if (childAt.getId() != C0616R.C0619id.circle_center && !shouldSkipView(childAt)) {
                int level = (Integer) childAt.getTag(C0616R.C0619id.material_clock_level);
                if (level == null) {
                    level = 1;
                }
    ...
    private void addConstraints(List<View> views, ConstraintSet constraintSet, int leveledRadius) {
        ...
        for (View view : views) {
            constraintSet.constrainCircle(view.getId(), C0616R.C0619id.circle_center, leveledRadius, currentAngle);
