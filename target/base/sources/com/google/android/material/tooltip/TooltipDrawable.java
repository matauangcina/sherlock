...
public class TooltipDrawable extends MaterialShapeDrawable implements TextDrawableHelper.TextDrawableDelegate {
    private static final int DEFAULT_STYLE = C0616R.style.Widget_MaterialComponents_Tooltip;
    private static final int DEFAULT_THEME_ATTR = C0616R.attr.tooltipStyle;
    ...
    private void loadFromAttributes(AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        TypedArray a = ThemeEnforcement.obtainStyledAttributes(this.context, attrs, C0616R.styleable.Tooltip, defStyleAttr, defStyleRes, new int[0]);
        this.arrowSize = this.context.getResources().getDimensionPixelSize(C0616R.dimen.mtrl_tooltip_arrowSize);
        setShapeAppearanceModel(getShapeAppearanceModel().toBuilder().setBottomEdge(createMarkerEdge()).build());
        setText(a.getText(C0616R.styleable.Tooltip_android_text));
        TextAppearance textAppearance = MaterialResources.getTextAppearance(this.context, a, C0616R.styleable.Tooltip_android_textAppearance);
        if (textAppearance != null && a.hasValue(C0616R.styleable.Tooltip_android_textColor)) {
            textAppearance.setTextColor(MaterialResources.getColorStateList(this.context, a, C0616R.styleable.Tooltip_android_textColor));
        }
        setTextAppearance(textAppearance);
        int onBackground = MaterialColors.getColor(this.context, C0616R.attr.colorOnBackground, TooltipDrawable.class.getCanonicalName());
        ...
        setFillColor(ColorStateList.valueOf(a.getColor(C0616R.styleable.Tooltip_backgroundTint, backgroundTintDefault)));
        setStrokeColor(ColorStateList.valueOf(MaterialColors.getColor(this.context, C0616R.attr.colorSurface, TooltipDrawable.class.getCanonicalName())));
        this.padding = a.getDimensionPixelSize(C0616R.styleable.Tooltip_android_padding, 0);
        this.minWidth = a.getDimensionPixelSize(C0616R.styleable.Tooltip_android_minWidth, 0);
        this.minHeight = a.getDimensionPixelSize(C0616R.styleable.Tooltip_android_minHeight, 0);
        this.layoutMargin = a.getDimensionPixelSize(C0616R.styleable.Tooltip_android_layout_margin, 0);
        a.recycle();
    }
