...
public class RangeDateSelector implements DateSelector<Pair<Long, Long>> {
    ...
    public int getDefaultThemeResId(Context context) {
        ...
        int maximumDefaultFullscreenMinorAxis = res.getDimensionPixelSize(C0616R.dimen.mtrl_calendar_maximum_default_fullscreen_minor_axis);
        int minorAxisPx = Math.min(display.widthPixels, display.heightPixels);
        if (minorAxisPx > maximumDefaultFullscreenMinorAxis) {
            defaultThemeAttr = C0616R.attr.materialCalendarTheme;
        } else {
            defaultThemeAttr = C0616R.attr.materialCalendarFullscreenTheme;
        }
    ...
    public String getSelectionDisplayString(Context context) {
        ...
        if (l == null && this.selectedEndItem == null) {
            return res.getString(C0616R.string.mtrl_picker_range_header_unselected);
        }
        Long l2 = this.selectedEndItem;
        if (l2 == null) {
            return res.getString(C0616R.string.mtrl_picker_range_header_only_start_selected, DateStrings.getDateString(this.selectedStartItem.longValue()));
        }
        if (l == null) {
            return res.getString(C0616R.string.mtrl_picker_range_header_only_end_selected, DateStrings.getDateString(this.selectedEndItem.longValue()));
        }
        Pair<String, String> dateRangeStrings = DateStrings.getDateRangeString(l, l2);
        return res.getString(C0616R.string.mtrl_picker_range_header_selected, dateRangeStrings.first, dateRangeStrings.second);
    }
    ...
    public String getSelectionContentDescription(Context context) {
        ...
        if (dateRangeStrings.first == null) {
            startPlaceholder = res.getString(C0616R.string.mtrl_picker_announce_current_selection_none);
        } else {
            startPlaceholder = dateRangeStrings.first;
        }
        if (dateRangeStrings.second == null) {
            endPlaceholder = res.getString(C0616R.string.mtrl_picker_announce_current_selection_none);
        } else {
            endPlaceholder = dateRangeStrings.second;
        }
        return res.getString(C0616R.string.mtrl_picker_announce_current_range_selection, startPlaceholder, endPlaceholder);
    }
    ...
    public int getDefaultTitleResId() {
        return C0616R.string.mtrl_picker_range_header_title;
    }
    ...
    public View onCreateTextInputView(LayoutInflater layoutInflater, ViewGroup viewGroup, Bundle bundle, CalendarConstraints constraints, final OnSelectionChangedListener<Pair<Long, Long>> listener) {
        ...
        View root = layoutInflater.inflate(C0616R.layout.mtrl_picker_text_input_date_range, viewGroup, false);
        final TextInputLayout startTextInput = (TextInputLayout) root.findViewById(C0616R.C0619id.mtrl_picker_text_input_range_start);
        final TextInputLayout endTextInput = (TextInputLayout) root.findViewById(C0616R.C0619id.mtrl_picker_text_input_range_end);
        ...
        this.invalidRangeStartError = root.getResources().getString(C0616R.string.mtrl_picker_invalid_range);
