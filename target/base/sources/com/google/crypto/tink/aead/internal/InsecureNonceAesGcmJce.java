...
public final class InsecureNonceAesGcmJce {
    ...
    public byte[] encrypt(final byte[] iv, final byte[] plaintext, final byte[] associatedData) throws GeneralSecurityException {
        ...
        int written = threadLocal.get().doFinal(plaintext, 0, plaintext.length, ciphertext, ciphertextOutputOffset);
    ...
    public byte[] decrypt(final byte[] iv, final byte[] ciphertext, final byte[] associatedData) throws GeneralSecurityException {
        ...
        boolean z = this.prependIv;
        int minimumCiphertextLength = z ? 28 : 16;
        ...
        int ciphertextInputOffset = z2 ? 12 : 0;
        int ciphertextLength = z2 ? ciphertext.length - 12 : ciphertext.length;
        return threadLocal.get().doFinal(ciphertext, ciphertextInputOffset, ciphertextLength);
    }
