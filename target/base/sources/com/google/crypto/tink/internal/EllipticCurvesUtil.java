...
public final class EllipticCurvesUtil {
    ...
    public static class JacobianEcPoint {
        ...
        boolean isInfinity() {
            return this.f181z.equals(BigInteger.ZERO);
        }
        ...
        ECPoint toECPoint(BigInteger modulus) {
            ...
            BigInteger zInv = this.f181z.modInverse(modulus);
            BigInteger zInv2 = zInv.multiply(zInv).mod(modulus);
            return new ECPoint(this.f179x.multiply(zInv2).mod(modulus), this.f180y.multiply(zInv2).mod(modulus).multiply(zInv).mod(modulus));
        }
    }
    ...
    static JacobianEcPoint doubleJacobianPoint(JacobianEcPoint p, BigInteger a, BigInteger modulus) {
        if (p.f180y.equals(BigInteger.ZERO)) {
            return JacobianEcPoint.INFINITY;
        }
        BigInteger xx = p.f179x.multiply(p.f179x).mod(modulus);
        BigInteger yy = p.f180y.multiply(p.f180y).mod(modulus);
        BigInteger yyyy = yy.multiply(yy).mod(modulus);
        BigInteger zz = p.f181z.multiply(p.f181z).mod(modulus);
        BigInteger x1yy = p.f179x.add(yy);
        BigInteger subtract = x1yy.multiply(x1yy).mod(modulus).subtract(xx).subtract(yyyy);
        BigInteger bigInteger = TWO;
        BigInteger s = subtract.multiply(bigInteger);
        BigInteger m = xx.multiply(THREE).add(a.multiply(zz).multiply(zz).mod(modulus));
        BigInteger t = m.multiply(m).mod(modulus).subtract(s.multiply(bigInteger)).mod(modulus);
        BigInteger y3 = m.multiply(s.subtract(t)).mod(modulus).subtract(yyyy.multiply(EIGHT)).mod(modulus);
        BigInteger y1z1 = p.f180y.add(p.f181z);
    ...
    static JacobianEcPoint addJacobianPoints(JacobianEcPoint p1, JacobianEcPoint p2, BigInteger a, BigInteger modulus) {
        ...
        BigInteger z1z1 = p1.f181z.multiply(p1.f181z).mod(modulus);
        BigInteger z2z2 = p2.f181z.multiply(p2.f181z).mod(modulus);
        BigInteger u1 = p1.f179x.multiply(z2z2).mod(modulus);
        BigInteger u2 = p2.f179x.multiply(z1z1).mod(modulus);
        BigInteger s1 = p1.f180y.multiply(p2.f181z).mod(modulus).multiply(z2z2).mod(modulus);
        BigInteger s2 = p2.f180y.multiply(p1.f181z).mod(modulus).multiply(z1z1).mod(modulus);
        if (u1.equals(u2)) {
            if (s1.equals(s2)) {
        ...
        BigInteger subtract = s2.subtract(s1);
        BigInteger bigInteger = TWO;
        BigInteger r = subtract.multiply(bigInteger).mod(modulus);
        BigInteger v = u1.multiply(i).mod(modulus);
        BigInteger x3 = r.multiply(r).mod(modulus).subtract(j).subtract(v.multiply(bigInteger)).mod(modulus);
        BigInteger y3 = r.multiply(v.subtract(x3)).subtract(s1.multiply(bigInteger).multiply(j)).mod(modulus);
        BigInteger z12 = p1.f181z.add(p2.f181z);
