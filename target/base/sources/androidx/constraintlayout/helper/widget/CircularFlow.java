    ...
    public void init(AttributeSet attrs) {
        ...
        if (attrs != null) {
            ...
            for (int i = 0; i < N; i++) {
                ...
                if (attr == C0177R.styleable.ConstraintLayout_Layout_circularflow_viewCenter) {
                    this.mViewCenter = a.getResourceId(attr, 0);
                } else if (attr == C0177R.styleable.ConstraintLayout_Layout_circularflow_angles) {
                    String string = a.getString(attr);
                    this.mReferenceAngles = string;
                    setAngles(string);
                } else if (attr == C0177R.styleable.ConstraintLayout_Layout_circularflow_radiusInDP) {
                    String string2 = a.getString(attr);
                    this.mReferenceRadius = string2;
                    setRadius(string2);
                } else if (attr == C0177R.styleable.ConstraintLayout_Layout_circularflow_defaultAngle) {
                    Float valueOf = Float.valueOf(a.getFloat(attr, DEFAULT_ANGLE));
                    this.mReferenceDefaultAngle = valueOf;
                    setDefaultAngle(valueOf.floatValue());
                } else if (attr == C0177R.styleable.ConstraintLayout_Layout_circularflow_defaultRadius) {
