...
public class MotionPaths implements Comparable<MotionPaths> {
    ...
    void initCartesian(MotionKeyPosition c, MotionPaths startTimePoint, MotionPaths endTimePoint) {
        ...
        float f5 = startTimePoint.f18x;
        float startCenterX = f5 + (f2 / 2.0f);
        float position2 = startTimePoint.f19y;
        float startCenterY = position2 + (f4 / 2.0f);
        float endCenterX = endTimePoint.f18x + (f / 2.0f);
        float endCenterY = endTimePoint.f19y + (f3 / 2.0f);
        float pathVectorX = endCenterX - startCenterX;
        float pathVectorY = endCenterY - startCenterY;
        this.f18x = (int) ((f5 + (pathVectorX * position)) - ((scaleX * scaleWidth) / 2.0f));
        this.f19y = (int) ((position2 + (pathVectorY * position)) - ((scaleY * scaleHeight) / 2.0f));
        ...
        this.mMode = 0;
        this.f18x = (int) (((startTimePoint.f18x + (pathVectorX * dxdx)) + (pathVectorY * dxdy)) - ((scaleX * scaleWidth) / 2.0f));
        this.f19y = (int) (((startTimePoint.f19y + (pathVectorX * dydx)) + (pathVectorY * dydy)) - ((scaleY * scaleHeight) / 2.0f));
    ...
    public void setupRelative(Motion mc, MotionPaths relative) {
        double dx = ((this.f18x + (this.width / 2.0f)) - relative.f18x) - (relative.width / 2.0f);
        double dy = ((this.f19y + (this.height / 2.0f)) - relative.f19y) - (relative.height / 2.0f);
        this.mRelativeToController = mc;
        this.f18x = (float) Math.hypot(dy, dx);
        if (Float.isNaN(this.mRelativeAngle)) {
            this.f19y = (float) (Math.atan2(dy, dx) + 1.5707963267948966d);
        } else {
    ...
    void initScreen(int parentWidth, int parentHeight, MotionKeyPosition c, MotionPaths startTimePoint, MotionPaths endTimePoint) {
        ...
        this.mMode = 2;
        if (!Float.isNaN(c.mPercentX)) {
            this.f18x = (int) (c.mPercentX * ((int) (parentWidth - this.width)));
        }
        if (!Float.isNaN(c.mPercentY)) {
            this.f19y = (int) (c.mPercentY * ((int) (parentHeight - this.height)));
        }
    ...
    void initPath(MotionKeyPosition c, MotionPaths startTimePoint, MotionPaths endTimePoint) {
        ...
        float startCenterX = (startTimePoint.width / 2.0f) + startTimePoint.f18x;
        float startCenterY = startTimePoint.f19y + (startTimePoint.height / 2.0f);
        float endCenterX = endTimePoint.f18x + (endTimePoint.width / 2.0f);
        float endCenterY = endTimePoint.f19y + (endTimePoint.height / 2.0f);
        float pathVectorX = endCenterX - startCenterX;
        float pathVectorY = endCenterY - startCenterY;
        this.f18x = (int) ((f + (pathVectorX * path)) - ((scaleX * scaleWidth) / 2.0f));
        this.f19y = (int) ((f3 + (pathVectorY * path)) - ((scaleY * scaleHeight) / 2.0f));
        this.width = (int) (f2 + (scaleX * scaleWidth));
        ...
        this.mMode = 1;
        float endCenterY2 = startTimePoint.f18x;
        float f4 = (int) ((endCenterY2 + (pathVectorX * path)) - ((scaleX * scaleWidth) / 2.0f));
        this.f18x = f4;
        float f5 = (int) ((startTimePoint.f19y + (pathVectorY * path)) - ((scaleY * scaleHeight) / 2.0f));
        this.f19y = f5;
        this.f18x = f4 + normalX;
        this.f19y = f5 + normalY;
        this.mAnimateRelativeTo = this.mAnimateRelativeTo;
        this.mKeyFrameEasing = Easing.getInterpolator(c.mTransitionEasing);
        this.mPathMotionArc = c.mPathMotionArc;
    }
    ...
    public void different(MotionPaths points, boolean[] mask, String[] custom, boolean arcMode) {
        boolean diffx = diff(this.f18x, points.f18x);
        boolean diffy = diff(this.f19y, points.f19y);
        int c = 0 + 1;
    ...
    public void setView(float position, MotionWidget view, int[] toUse, double[] data, double[] slope, double[] cycle) {
        ...
        float v_x2 = this.f18x;
        float v_y3 = this.f19y;
        ...
        while (true) {
            ...
            if (i2 < dArr.length) {
                ...
                if (motion != null) {
                    ...
                    float drx = vel[0];
                    float v_y5 = vel[1];
                    float pos_x = (float) ((rx + (radius * Math.sin(angle))) - (v_width3 / 2.0f));
                    float pos_y = (float) ((ry - (radius * Math.cos(angle))) - (v_height2 / 2.0f));
                    double d = dradius;
                    v_width = v_width3;
                    v_height = v_height2;
                    float dpos_x = (float) (drx + (d * Math.sin(angle)) + (radius * Math.cos(angle) * dangle));
                    float dpos_y = (float) ((v_y5 - (dradius * Math.cos(angle))) + (radius * Math.sin(angle) * dangle));
                    v_x = pos_x;
                    if (slope.length >= 2) {
                        dry = v_y5;
                        slope[0] = dpos_x;
    ...
    public void fillStandard(double[] data, int[] toUse) {
        float[] set = {this.position, this.f18x, this.f19y, this.width, this.height, this.mPathRotate};
        int c = 0;
        for (int i = 0; i < toUse.length; i++) {
