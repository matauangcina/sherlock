...
public class PathInterpolatorCompat implements Interpolator {
    ...
    /* renamed from: mX */
    private float[] f106mX;

    /* renamed from: mY */
    private float[] f107mY;
    ...
    private void initPath(Path path) {
        ...
        this.f106mX = new float[numPoints];
        this.f107mY = new float[numPoints];
        float[] position = new float[2];
        for (int i = 0; i < numPoints; i++) {
            float distance = (i * pathLength) / (numPoints - 1);
            pathMeasure.getPosTan(distance, position, null);
            this.f106mX[i] = position[0];
            this.f107mY[i] = position[1];
        }
        if (Math.abs(this.f106mX[0]) > 1.0E-5d || Math.abs(this.f107mY[0]) > 1.0E-5d || Math.abs(this.f106mX[numPoints - 1] - 1.0f) > 1.0E-5d || Math.abs(this.f107mY[numPoints - 1] - 1.0f) > 1.0E-5d) {
            throw new IllegalArgumentException("The Path must start at (0,0) and end at (1,1) start: " + this.f106mX[0] + "," + this.f107mY[0] + " end:" + this.f106mX[numPoints - 1] + "," + this.f107mY[numPoints - 1]);
        }
        float prevX = 0.0f;
    ...
    public float getInterpolation(float t) {
        ...
        int startIndex = 0;
        int endIndex = this.f106mX.length - 1;
        while (endIndex - startIndex > 1) {
            int midIndex = (startIndex + endIndex) / 2;
            if (t < this.f106mX[midIndex]) {
                endIndex = midIndex;
            } else {
                startIndex = midIndex;
            }
