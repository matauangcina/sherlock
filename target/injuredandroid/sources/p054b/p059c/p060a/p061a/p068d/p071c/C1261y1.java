package p054b.p059c.p060a.p061a.p068d.p071c;

import android.os.Parcel;
import android.os.Parcelable;
import com.google.android.gms.common.internal.p117w.C1685b;
/* renamed from: b.c.a.a.d.c.y1 */
/* loaded from: classes.dex */
public final class C1261y1 implements Parcelable.Creator<C1258x1> {
    @Override // android.os.Parcelable.Creator
    public final /* synthetic */ C1258x1 createFromParcel(Parcel parcel) {
        int m3600r = C1685b.m3600r(parcel);
        String str = null;
        String str2 = null;
        String str3 = null;
        C1243s1 c1243s1 = null;
        while (parcel.dataPosition() < m3600r) {
            int m3607k = C1685b.m3607k(parcel);
            int m3610h = C1685b.m3610h(m3607k);
            if (m3610h == 2) {
                str = C1685b.m3615c(parcel, m3607k);
            } else if (m3610h == 3) {
                str2 = C1685b.m3615c(parcel, m3607k);
            } else if (m3610h == 4) {
                str3 = C1685b.m3615c(parcel, m3607k);
            } else if (m3610h != 5) {
                C1685b.m3601q(parcel, m3607k);
            } else {
                c1243s1 = (C1243s1) C1685b.m3616b(parcel, m3607k, C1243s1.CREATOR);
            }
        }
        C1685b.m3611g(parcel, m3600r);
        return new C1258x1(str, str2, str3, c1243s1);
    }

    @Override // android.os.Parcelable.Creator
    public final /* synthetic */ C1258x1[] newArray(int i) {
        return new C1258x1[i];
    }
}
