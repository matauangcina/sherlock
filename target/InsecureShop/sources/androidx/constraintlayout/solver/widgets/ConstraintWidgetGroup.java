...
public class ConstraintWidgetGroup {
    ...
    private void updateResolvedDimension(ConstraintWidget widget) {
        ...
        if (rightSide) {
            ...
            ConstraintAnchor targetAnchor3 = widget.mLeft;
            targetAnchor = targetAnchor3.mTarget;
        }
        if (targetAnchor != null) {
            ...
            if (targetAnchor.mType == ConstraintAnchor.Type.RIGHT) {
                end3 = targetAnchor.mOwner.f15mX + targetAnchor.mOwner.getWidth();
            } else if (targetAnchor.mType == ConstraintAnchor.Type.LEFT) {
                end3 = targetAnchor.mOwner.f15mX;
            }
        }
        if (rightSide) {
        ...
        if (widget.mBaseline.mTarget != null) {
            ConstraintAnchor targetAnchor4 = widget.mBaseline.mTarget;
            if (!targetAnchor4.mOwner.mOptimizerMeasured) {
                updateResolvedDimension(targetAnchor4.mOwner);
            }
            int start2 = (targetAnchor4.mOwner.f16mY + targetAnchor4.mOwner.mBaselineDistance) - widget.mBaselineDistance;
        ...
        if (targetAnchor2 != null) {
            ...
            if (targetAnchor2.mType == ConstraintAnchor.Type.BOTTOM) {
                end = targetAnchor2.mOwner.f16mY + targetAnchor2.mOwner.getHeight();
            } else if (targetAnchor2.mType == ConstraintAnchor.Type.TOP) {
                end = targetAnchor2.mOwner.f16mY;
            }
        }
        if (bottomSide) {
