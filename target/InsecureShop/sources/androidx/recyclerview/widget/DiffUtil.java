...
public class DiffUtil {
    private static final Comparator<Snake> SNAKE_COMPARATOR = new Comparator<Snake>() { // from class: androidx.recyclerview.widget.DiffUtil.1
        ...
        public int compare(Snake o1, Snake o2) {
            int cmpX = o1.f33x - o2.f33x;
            return cmpX == 0 ? o1.f34y - o2.f34y : cmpX;
        }
    };
    ...
    public static DiffResult calculateDiff(Callback cb, boolean detectMoves) {
        ...
        while (!stack.isEmpty()) {
            ...
            if (snake != null) {
                ...
                snake.f33x += range.oldListStart;
                snake.f34y += range.newListStart;
                Range left = rangePool.isEmpty() ? new Range() : rangePool.remove(rangePool.size() - 1);
                left.oldListStart = range.oldListStart;
                left.newListStart = range.newListStart;
                if (snake.reverse) {
                    left.oldListEnd = snake.f33x;
                    left.newListEnd = snake.f34y;
                } else if (snake.removal) {
                    left.oldListEnd = snake.f33x - 1;
                    left.newListEnd = snake.f34y;
                } else {
                    left.oldListEnd = snake.f33x;
                    left.newListEnd = snake.f34y - 1;
                }
                stack.add(left);
                if (!snake.reverse) {
                    range.oldListStart = snake.f33x + snake.size;
                    range.newListStart = snake.f34y + snake.size;
                } else if (snake.removal) {
                    range.oldListStart = snake.f33x + snake.size + 1;
                    range.newListStart = snake.f34y + snake.size;
                } else {
                    range.oldListStart = snake.f33x + snake.size;
                    range.newListStart = snake.f34y + snake.size + 1;
                }
                stack.add(range);
            } else {
                rangePool.add(range);
            ...
            for (int d = 0; d <= dLimit; d++) {
                for (int k = -d; k <= d; k += 2) {
                    ...
                    if (checkInFwd && k >= (delta - d) + 1 && k <= (delta + d) - 1 && forward[kOffset + k] >= backward[kOffset + k]) {
                        ...
                        outSnake.f33x = backward[kOffset + k];
                        outSnake.f34y = outSnake.f33x - k;
                ...
                while (k2 <= d) {
                    ...
                    if (checkInFwd || k2 + delta < (-d) || k2 + delta > d || forward[kOffset + backwardK] < backward[kOffset + backwardK]) {
                        ...
                        outSnake2.f33x = backward[kOffset + backwardK];
                        outSnake2.f34y = outSnake2.f33x - backwardK;
    ...
    public static class DiffResult {
        ...
        private void addRootSnake() {
            ...
            if (firstSnake == null || firstSnake.f33x != 0 || firstSnake.f34y != 0) {
        ...
        private void findMatchingItems() {
            ...
            for (int i = this.mSnakes.size() - 1; i >= 0; i--) {
                ...
                int endX = snake.f33x + snake.size;
                int endY = snake.f34y + snake.size;
                ...
                for (int j = 0; j < snake.size; j++) {
                    int oldItemPos = snake.f33x + j;
                    int newItemPos = snake.f34y + j;
        ...
        public void dispatchUpdatesTo(ListUpdateCallback updateCallback) {
            ...
            for (int snakeIndex = this.mSnakes.size() - 1; snakeIndex >= 0; snakeIndex--) {
                ...
                int endX = snake.f33x + snakeSize2;
                int endY2 = snake.f34y + snakeSize2;
                if (endX >= posOld2) {
                    endY = endY2;
                } else {
                    endY = endY2;
                ...
                for (int i = snakeSize - 1; i >= 0; i--) {
                    if ((this.mOldItemStatuses[snake.f33x + i] & 31) == 2) {
                        batchingCallback.onChanged(snake.f33x + i, 1, this.mCallback.getChangePayload(snake.f33x + i, snake.f34y + i));
                    }
                }
                posOld2 = snake.f33x;
                posNew2 = snake.f34y;
