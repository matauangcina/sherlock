...
public final class DefaultImageHeaderParser implements ImageHeaderParser {
    ...
    private static final class StreamReader implements Reader {
        ...
        /* renamed from: is */
        private final InputStream f62is;
        ...
        public short getUInt8() throws IOException {
            int readResult = this.f62is.read();
            if (readResult == -1) {
                throw new Reader.EndOfFileException();
            }
        ...
        public int read(byte[] buffer, int byteCount) throws IOException {
            ...
            while (numBytesRead < byteCount) {
                int read = this.f62is.read(buffer, numBytesRead, byteCount - numBytesRead);
                lastReadResult = read;
                if (read == -1) {
                    break;
                }
        ...
        public long skip(long total) throws IOException {
            ...
            long toSkip = total;
            while (toSkip > 0) {
                long skipped = this.f62is.skip(toSkip);
                if (skipped > 0) {
                    toSkip -= skipped;
                } else {
                    int testEofByte = this.f62is.read();
                    if (testEofByte == -1) {
                        break;
                    }
                    toSkip--;
