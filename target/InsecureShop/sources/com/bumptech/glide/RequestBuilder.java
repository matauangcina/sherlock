...
public class RequestBuilder<TranscodeType> extends BaseRequestOptions<RequestBuilder<TranscodeType>> implements Cloneable, ModelTypes<RequestBuilder<TranscodeType>> {
    ...
    @Override // com.bumptech.glide.request.BaseRequestOptions
    /* renamed from: clone */
    public RequestBuilder<TranscodeType> mo47clone() {
        RequestBuilder<TranscodeType> result = (RequestBuilder) super.mo47clone();
        result.transitionOptions = (TransitionOptions<?, ? super TranscodeType>) result.transitionOptions.m48clone();
        return result;
    }
    ...
    public ViewTarget<ImageView, TranscodeType> into(ImageView view) {
        ...
        if (!requestOptions.isTransformationSet() && requestOptions.isTransformationAllowed() && view.getScaleType() != null) {
            switch (C05651.$SwitchMap$android$widget$ImageView$ScaleType[view.getScaleType().ordinal()]) {
                case 1:
                    requestOptions = requestOptions.mo47clone().optionalCenterCrop();
                    break;
                case 2:
                    requestOptions = requestOptions.mo47clone().optionalCenterInside();
                    break;
                case 3:
                case 4:
                case 5:
                    requestOptions = requestOptions.mo47clone().optionalFitCenter();
                    break;
                case 6:
                    requestOptions = requestOptions.mo47clone().optionalCenterInside();
                    break;
    ...
    private Priority getThumbnailPriority(Priority current) {
        int i = C05651.$SwitchMap$com$bumptech$glide$Priority[current.ordinal()];
        if (i != 1) {
            if (i != 2) {
                if (i == 3 || i == 4) {
    ...
    private Request buildThumbnailRequestRecursive(Object requestLock, Target<TranscodeType> target, RequestListener<TranscodeType> targetListener, RequestCoordinator parentCoordinator, TransitionOptions<?, ? super TranscodeType> transitionOptions, Priority priority, int overrideWidth, int overrideHeight, BaseRequestOptions<?> requestOptions, Executor callbackExecutor) {
        ...
        if (requestBuilder == null) {
            if (this.thumbSizeMultiplier != null) {
                ...
                BaseRequestOptions<?> thumbnailOptions = requestOptions.mo47clone().sizeMultiplier(this.thumbSizeMultiplier.floatValue());
