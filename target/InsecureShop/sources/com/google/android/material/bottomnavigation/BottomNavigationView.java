...
public class BottomNavigationView extends FrameLayout {
    private static final int DEF_STYLE_RES = C0630R.style.Widget_Design_BottomNavigationView;
    ...
    public BottomNavigationView(Context context, AttributeSet attrs) {
        this(context, attrs, C0630R.attr.bottomNavigationStyle);
    }
    ...
    public BottomNavigationView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TintTypedArray a = ThemeEnforcement.obtainTintedStyledAttributes(context2, attrs, C0630R.styleable.BottomNavigationView, defStyleAttr, C0630R.style.Widget_Design_BottomNavigationView, C0630R.styleable.BottomNavigationView_itemTextAppearanceInactive, C0630R.styleable.BottomNavigationView_itemTextAppearanceActive);
        if (a.hasValue(C0630R.styleable.BottomNavigationView_itemIconTint)) {
            this.menuView.setIconTintList(a.getColorStateList(C0630R.styleable.BottomNavigationView_itemIconTint));
        } else {
            BottomNavigationMenuView bottomNavigationMenuView = this.menuView;
            bottomNavigationMenuView.setIconTintList(bottomNavigationMenuView.createDefaultColorStateList(16842808));
        }
        setItemIconSize(a.getDimensionPixelSize(C0630R.styleable.BottomNavigationView_itemIconSize, getResources().getDimensionPixelSize(C0630R.dimen.design_bottom_navigation_icon_size)));
        if (a.hasValue(C0630R.styleable.BottomNavigationView_itemTextAppearanceInactive)) {
            setItemTextAppearanceInactive(a.getResourceId(C0630R.styleable.BottomNavigationView_itemTextAppearanceInactive, 0));
        }
        if (a.hasValue(C0630R.styleable.BottomNavigationView_itemTextAppearanceActive)) {
            setItemTextAppearanceActive(a.getResourceId(C0630R.styleable.BottomNavigationView_itemTextAppearanceActive, 0));
        }
        if (a.hasValue(C0630R.styleable.BottomNavigationView_itemTextColor)) {
            setItemTextColor(a.getColorStateList(C0630R.styleable.BottomNavigationView_itemTextColor));
        }
        if (getBackground() == null || (getBackground() instanceof ColorDrawable)) {
            ViewCompat.setBackground(this, createMaterialShapeDrawableBackground(context2));
        }
        if (a.hasValue(C0630R.styleable.BottomNavigationView_elevation)) {
            ViewCompat.setElevation(this, a.getDimensionPixelSize(C0630R.styleable.BottomNavigationView_elevation, 0));
        }
        ColorStateList backgroundTint = MaterialResources.getColorStateList(context2, a, C0630R.styleable.BottomNavigationView_backgroundTint);
        DrawableCompat.setTintList(getBackground().mutate(), backgroundTint);
        setLabelVisibilityMode(a.getInteger(C0630R.styleable.BottomNavigationView_labelVisibilityMode, -1));
        setItemHorizontalTranslationEnabled(a.getBoolean(C0630R.styleable.BottomNavigationView_itemHorizontalTranslationEnabled, true));
        int itemBackground = a.getResourceId(C0630R.styleable.BottomNavigationView_itemBackground, 0);
        if (itemBackground != 0) {
            this.menuView.setItemBackgroundRes(itemBackground);
        } else {
            ColorStateList itemRippleColor = MaterialResources.getColorStateList(context2, a, C0630R.styleable.BottomNavigationView_itemRippleColor);
            setItemRippleColor(itemRippleColor);
        }
        if (a.hasValue(C0630R.styleable.BottomNavigationView_menu)) {
            inflateMenu(a.getResourceId(C0630R.styleable.BottomNavigationView_menu, 0));
        }
        a.recycle();
    ...
    private void addCompatibilityTopDivider(Context context) {
        ...
        divider.setBackgroundColor(ContextCompat.getColor(context, C0630R.C0631color.design_bottom_navigation_shadow_color));
        FrameLayout.LayoutParams dividerParams = new FrameLayout.LayoutParams(-1, getResources().getDimensionPixelSize(C0630R.dimen.design_bottom_navigation_shadow_height));
