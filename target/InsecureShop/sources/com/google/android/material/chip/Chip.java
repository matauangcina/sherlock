...
public class Chip extends AppCompatCheckBox implements ChipDrawable.Delegate, Shapeable {
    ...
    public Chip(Context context, AttributeSet attrs) {
        this(context, attrs, C0630R.attr.chipStyle);
    }
    ...
    public Chip(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        ChipDrawable drawable = ChipDrawable.createFromAttributes(context, attrs, defStyleAttr, C0630R.style.Widget_MaterialComponents_Chip_Action);
        initMinTouchTarget(context, attrs, defStyleAttr);
        setChipDrawable(drawable);
        drawable.setElevation(ViewCompat.getElevation(this));
        TypedArray a = ThemeEnforcement.obtainStyledAttributes(context, attrs, C0630R.styleable.Chip, defStyleAttr, C0630R.style.Widget_MaterialComponents_Chip_Action, new int[0]);
        if (Build.VERSION.SDK_INT < 23) {
            setTextColor(MaterialResources.getColorStateList(context, a, C0630R.styleable.Chip_android_textColor));
        }
        boolean hasShapeAppearanceAttribute = a.hasValue(C0630R.styleable.Chip_shapeAppearance);
        a.recycle();
    ...
    private void initMinTouchTarget(Context context, AttributeSet attrs, int defStyleAttr) {
        TypedArray a = ThemeEnforcement.obtainStyledAttributes(context, attrs, C0630R.styleable.Chip, defStyleAttr, C0630R.style.Widget_MaterialComponents_Chip_Action, new int[0]);
        this.ensureMinTouchTargetSize = a.getBoolean(C0630R.styleable.Chip_ensureMinTouchTargetSize, false);
        float defaultMinTouchTargetSize = (float) Math.ceil(ViewUtils.dpToPx(getContext(), 48));
        this.minTouchTargetSize = (int) Math.ceil(a.getDimension(C0630R.styleable.Chip_chipMinTouchTargetSize, defaultMinTouchTargetSize));
        a.recycle();
    }
        ...
        protected void onPopulateNodeForVirtualView(int virtualViewId, AccessibilityNodeInfoCompat node) {
            if (virtualViewId == 1) {
                ...
                if (closeIconContentDescription != null) {
                    ...
                    int i = C0630R.string.mtrl_chip_close_icon_content_description;
