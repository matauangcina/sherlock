...
public final class Buffer implements BufferedSource, BufferedSink, Cloneable {
    ...
    public short readShort() {
        ...
        int limit = segment.limit;
        if (limit - pos < 2) {
            int s = ((readByte() & UByte.MAX_VALUE) << 8) | (readByte() & UByte.MAX_VALUE);
            return (short) s;
        }
        byte[] data = segment.data;
        int pos2 = pos + 1;
    ...
    public long readLong() {
        ...
        long v = ((data[pos8] & 255) << 56) | ((data[pos] & 255) << 48) | ((data[pos2] & 255) << 40) | ((data[pos3] & 255) << 32) | ((data[pos4] & 255) << 24) | ((data[pos5] & 255) << 16) | ((data[pos6] & 255) << 8) | (data[pos7] & 255);
        this.size -= 8;
        if (pos9 == limit) {
            this.head = segment.pop();
            SegmentPool.recycle(segment);
    ...
    */
    ...
    public int readUtf8CodePoint() throws EOFException {
        ...
        byte b0 = getByte(0L);
        if ((b0 & ByteCompanionObject.MIN_VALUE) == 0) {
        ...
        while (byteCount > 0) {
            if (byteCount < source.head.limit - source.head.pos) {
                ...
                if (tail != null && tail.owner) {
                    if ((tail.limit + byteCount) - (tail.shared ? 0 : tail.pos) <= 8192) {
    ...
    */
    ...
    public long indexOfElement(ByteString targetBytes, long fromIndex) {
        ...
        while (offset3 < buffer.size) {
            byte[] data2 = s.data;
            int pos3 = (int) ((s.pos + fromIndex3) - offset3);
            int limit2 = s.limit;
            while (pos3 < limit2) {
                byte b2 = data2[pos3];
    ...
    private ByteString digest(String algorithm) {
        try {
            ...
            return ByteString.m3of(messageDigest.digest());
    ...
    private ByteString hmac(String algorithm, ByteString key) {
        try {
            ...
            return ByteString.m3of(mac.doFinal());
