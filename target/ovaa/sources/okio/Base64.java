...
final class Base64 {
    ...
    private static String encode(byte[] in, byte[] map) {
        ...
        for (int i = 0; i < end; i += 3) {
            int index2 = index + 1;
            out[index] = map[(in[i] & UByte.MAX_VALUE) >> 2];
            int index3 = index2 + 1;
            out[index2] = map[((in[i] & 3) << 4) | ((in[i + 1] & UByte.MAX_VALUE) >> 4)];
            int index4 = index3 + 1;
            out[index3] = map[((in[i + 1] & 15) << 2) | ((in[i + 2] & UByte.MAX_VALUE) >> 6)];
            index = index4 + 1;
        ...
        switch (i2 % 3) {
            case 1:
                int index5 = index + 1;
                out[index] = map[(in[end] & UByte.MAX_VALUE) >> 2];
                int index6 = index5 + 1;
                out[index5] = map[(in[end] & 3) << 4];
                int index7 = index6 + 1;
                out[index6] = kotlin.p004io.encoding.Base64.padSymbol;
                int i3 = index7 + 1;
                out[index7] = kotlin.p004io.encoding.Base64.padSymbol;
                break;
            case 2:
                int index8 = index + 1;
                out[index] = map[(in[end] & UByte.MAX_VALUE) >> 2];
                int index9 = index8 + 1;
                out[index8] = map[((in[end] & 3) << 4) | ((in[end + 1] & UByte.MAX_VALUE) >> 4)];
                int index10 = index9 + 1;
                out[index9] = map[(in[end + 1] & 15) << 2];
                int i4 = index10 + 1;
                out[index10] = kotlin.p004io.encoding.Base64.padSymbol;
                break;
        }
        try {
