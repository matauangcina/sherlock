...
public class LinearSystem {
    ...
    public SolverVariable createObjectVariable(Object anchor) {
        ...
        if (anchor instanceof ConstraintAnchor) {
            ...
            if (variable.f13id == -1 || variable.f13id > this.mVariablesID || this.mCache.mIndexedVariables[variable.f13id] == null) {
                if (variable.f13id != -1) {
                    variable.reset();
                }
    ...
    private final int optimize(Row goal, boolean b) {
        ...
        while (!done) {
            ...
            if (goal.getKey() != null) {
                this.mAlreadyTestedCandidates[goal.getKey().f13id] = true;
            }
                                ...
                                while (true) {
                                    ...
                                    if (k < 9) {
                                        ...
                                        if ((value < min && k == strength) || k > strength) {
                                            min = value;
                                            pivotRowIndex = i2;
                                            pivotColumnIndex = candidate.f13id;
                                            strength = k;
                                        }
                                        k++;
    ...
    public void displayReadableRows() {
        ...
        for (int i2 = 0; i2 < this.mVariablesID + 1; i2++) {
            ...
            if (variable2 != null && variable2.isSynonym) {
                ...
                s2 = s2 + " ~[" + i2 + "] => " + variable2 + " = " + synonym + " + " + variable2.synonymDelta + IOUtils.LINE_SEPARATOR_UNIX;
            }
        }
        String s3 = s2 + "\n\n #  ";
    ...
    public void addEquality(SolverVariable a, int value) {
        if (USE_BASIC_SYNONYMS && a.definitionId == -1) {
            ...
            for (int i = 0; i < this.mVariablesID + 1; i++) {
                SolverVariable variable = this.mCache.mIndexedVariables[i];
                if (variable != null && variable.isSynonym && variable.synonym == a.f13id) {
